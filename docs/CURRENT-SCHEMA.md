...
Dazu zeigt die Sektion seit 31.08.2026 die **drei Musterseiten** (Café Klee, Restaurant Glut, Salon Wirbel) als Dreierreihe unter den Live-Karten – Daten NICHT dupliziert, sondern `demoPreviews` aus `src/data/demos.ts` (Server-Import in `page.tsx`, als Prop `muster` an die Client-Sektion `Projekte` – `demos.ts` bleibt server-only, in der Client-Komponente steht nur `import type`; die Hero-Bühne zeigt ein fiktives, gezeichnetes Interface und braucht keine Projektdaten). Jede Karte trägt ihre Branche als Akzent-Eyebrow und ein Badge („Live im Netz" / „Musterseite") auf dem Screenshot. Die ganze Karte ist der Link (Muster von `DemoShowcase`); Live extern in neuem Tab, Musterseiten intern zu `/demo/*`. KEINE Detail-Ansicht/Overlay (geparkt seit 09.06.2026).
...
| Demo-Fotos (`/demo/friseur`) | ✅ (27.08.2026) **vollständig, zehn Bilder, kein Platzhalter mehr.** `friseur-platz.webp` 941×1176 (4∶5, 50 KB, Hero – zweite Fassung, löste `friseur-salon.webp` ab), `friseur-werkzeug.webp` 1080×1080 (121 KB), `friseur-waschplatz.webp` 1080×1080 (40 KB), `friseur-wartebank.webp` 1536×1024 (3∶2, 54 KB), `friseur-beratung.webp` 1200×800 (48 KB), `friseur-schnitt.webp` 868×1085 (4∶5, 38 KB), `friseur-spiegelreihe.webp` 1200×800 (45 KB – löste `friseur-spiegelwand.webp` ab, das aus der Serie fiel). Dazu die drei Team-Porträts, seit Session 27 in zweiter Fassung (die erste las sich als dreimal dieselbe Person): `friseur-team-mira.webp` 1254×1254 (46 KB), `friseur-team-jonte.webp` 1254×1254 (56 KB) und `friseur-team-aylin.webp` 1254×1254 (43 KB) – Quellen echtes Quadrat 1254×1254, deshalb **ungeschnitten und unskaliert** übernommen. Ihr Slot rendert 368 px, `DemoTeam` fordert seit Session 26 genau das an (vorher 372); auf DPR 3 ergibt das **1,09×** – die einzige Bildgruppe dieser Seiten über 1,0 auf Retina | `public/demo/` |
...
| `/demo/cafe`<br>`/demo/cafe/karte`<br>`/demo/cafe/ueber-uns`<br>`/demo/cafe/kontakt`<br>`/demo/cafe/reservieren`<br>`/demo/cafe/impressum`<br>`/demo/cafe/datenschutz` | Stille **Demo-Seiten** eines erfundenen Gastro-Betriebs („Café Klee"), seit 26.08.2026 mehrseitig, seit 03.09.2026 **siebenseitig**: Start (Kopf, Laufband, Bildreihe, Karten-Auszug) · Karte (volle Speisekarte mit Kategorie-Leiste) · Über uns (Lead + drei Blöcke im Bild-Text-Wechsel) · Kontakt (Adresse mit Routen-Link, Öffnungszeiten, Telefon, Reservierungs-Mail) · **Reservieren (Attrappe, 03.09.2026 – Stundenraster, Ruhetag Montag aus `hours` abgeleitet)** · Impressum · Datenschutz. Die ersten vier stehen in der Navigations-Pille, die zwei Rechtsseiten nur im Fuß; die Buchung hängt am Knopf im Navigationsband (ab 640 px), an der klebenden Buchungsleiste unten (darunter), am Kopf-CTA und am Einstieg auf /kontakt. Segmente rein ASCII. Alle: `noindex`, NICHT in der Sitemap. Eigene Layout-Gruppe `(demo)` → **keine** Navbar/Footer/ScrollProgress/Analytics, eigene helle Design-Tokens. Navigation und Impressum-Fuß liefert `DemoShell`. Siehe „Layout-Gruppen" unten |
| `/demo/restaurant`<br>`/demo/restaurant/karte`<br>`/demo/restaurant/ueber-uns`<br>`/demo/restaurant/kontakt`<br>`/demo/restaurant/impressum`<br>`/demo/restaurant/datenschutz` | **Zweite Demo** (seit 27.08.2026), erfundenes À-la-carte-Restaurant „Restaurant Glut". Gleiche Seiten, gleiche Hülle, gleiche Bausteine wie das Café – der Ordner ist eine Kopie von `demo/cafe/` mit anderem Import, die Inhalte kommen aus `src/data/demo/restaurant-glut.ts`. Unterschied zum Café: `/karte` trägt **drei Karten** (Mittagstisch, Abendkarte, Weinkarte), die Abendkarte ist in **Gänge** gegliedert und trägt zwei **Menü-Bündel** zum Festpreis. Seit 27.08. vollständig bebildert. **Seit 03.09.2026 eigener DUNKLER Token-Satz** (`theme: 'glut'` → `.demo-scope--glut`: verkohltes Braunschwarz + Glut-Orange, Kontraste gerechnet, `color-scheme: dark`, Korn auf `screen` gedreht, eigenes `viewport.themeColor` aus neuem `demo/restaurant/layout.tsx`) – vorher teilte Glut den hellen Basis-Satz des Cafés. Ebenfalls `noindex`, nicht in der Sitemap |
| `/demo/<slug>/reservieren` | **Buchungs-Attrappe – seit 03.09.2026 bei ALLEN vier Betrieben** (beim Restaurant seit 27.08.2026). Fünf Schritte, die **nichts tun**: kein Netzwerk-Request, keine Mail, kein Speichern. **Schritt 1 hat zwei Formen** (exklusive Union in `BookingDemo`): Personenzahl (`partySizes` – Café, Restaurant) ODER Leistungswahl (`choices` – Friseur, Barbier: Kacheln „Herrenschnitt · 45 Min. · 34 €"). Auf **jedem** Schritt als Vorschau gekennzeichnet (klebendes Band, Schrittzähler, ein je Schritt ANDERER Satz unter der Überschrift, ein Satz unter dem Weiter-Knopf, dazu „(Vorschau)" im Browser-Tab). Steht **nicht als fünftes Ziel in der Pille**, ist aber überall erreichbar: Knopf im Navigationsband (`.demo-nav-cta`, ab 640 px, Geschwister der Liste – die :has()-Höhenformel zählt nur `<li>`), klebende **Buchungsleiste** am unteren Rand (`.demo-buchungsleiste`, unter 640 px, `position: sticky` am Spaltenende → verdeckt am Seitenende nichts), Kopf-CTA der Startseite und Einstieg auf /kontakt. Die einzige Seite im Demo-Baum mit eigenem JavaScript; ohne JS erscheint an ihrer Stelle ein Erklärblock. Servicefenster-Zusätze („· Abend") erscheinen nur, wenn ein Tag MEHRERE Fenster hat. Bekannte Einschränkung: der **Browser-Zurück-Knopf verlässt die Strecke**, statt einen Schritt zurückzugehen – dagegen mit `history.pushState` anzusteuern kollidiert mit der History-Verwaltung des App-Routers |
| `/demo/friseur`<br>`/demo/friseur/karte`<br>`/demo/friseur/ueber-uns`<br>`/demo/friseur/kontakt`<br>`/demo/friseur/reservieren`<br>`/demo/friseur/impressum`<br>`/demo/friseur/datenschutz` | **Dritte** stille Demo (seit 27.08.2026): Friseursalon „Salon Wirbel" – der erste Betrieb, der **keine Gastronomie** ist. Dieselben Bausteine; **seit 03.09.2026 mit Termin-Attrappe** (erste im Leistungs-Modus: sechs buchbare Standards, alles mit Haarprobe verweist die `choicesNote` bewusst ans Telefon). Eigener Token-Satz (`theme: 'friseur'` → `.demo-scope--friseur`, kühles Porzellan + Pflaume), eigenes Icon-Paar („W"), eigenes `viewport.themeColor` aus `demo/friseur/layout.tsx`. Das Routen-Segment heisst weiterhin `karte`, die Seite überschrieben „Leistungen" — wie beim Restaurant „Karten". `noindex`, nicht in der Sitemap. |
| `/demo/barbier`<br>`/demo/barbier/karte`<br>`/demo/barbier/ueber-uns`<br>`/demo/barbier/kontakt`<br>`/demo/barbier/reservieren`<br>`/demo/barbier/impressum`<br>`/demo/barbier/datenschutz` | **Vierte** stille Demo (03.09.2026, User-Auftrag): Barbershop „Barbier Klinge" (`src/data/demo/barbier-klinge.ts`) – der Beleg, dass ein neuer Betrieb nur noch Datendatei + Ordnerkopie ist. Zweiter **dunkler** Token-Satz (`theme: 'barbier'` → `.demo-scope--barbier`: tiefes Flaschengrün + gealtertes Messing, Kontraste gerechnet), als Form-Akzent der **Rundbogen** über dem Kopf-Foto (`.demo-scope--barbier .demo-kopf__bild`, Barbier-Spiegel-Silhouette). Preisliste heisst „Preise", Termin-Attrappe im Leistungs-Modus, Team wie beim Friseur (drei erfundene Barbiere, Rückansichten). Zehn KI-generierte Fotos (`public/demo/barbier-*.webp`), jedes auf Schriftzüge/Marken/Gesichter abgesucht; im Wartebank-Foto hängt ein GEZEICHNETES Frisuren-Schaubild (Illustration, kein Porträt – geprüft und bewusst belassen). `seo.schemaType: 'BarberShop'` (Union + `PREISLISTEN_PROP` in `schema.ts` erweitert). Eigenes Icon-Paar („K"), `viewport.themeColor` aus `demo/barbier/layout.tsx`. `noindex`, nicht in der Sitemap. |
