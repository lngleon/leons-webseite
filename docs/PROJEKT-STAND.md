...
- **Build-/tsc-grün ≠ im-Browser-korrekt (08.06.2026):** Ein grüner `npm run build` (TypeScript + Vite + Prerender) garantiert NICHT, dass Render/Effekte im Browser stimmen – der AuroraText-Clip-Bug war build-grün UND trotzdem live unsichtbar. Visuelle/interaktive Features (Gradient-Clip, Tilt, Partikel, Marquee, reduced-motion, Hydration-Warnungen) brauchen echten Augenschein im Browser (lokal `npm run preview` + live nach Deploy), nicht nur einen grünen Build.
...
|-------|----------------|
| 09.06.2026 | **Session 17: a11y – Fokusring am mobilen Navbar-CTA.** Der CTA im Mobil-Menü (`src/components/Navbar.tsx`) war als EINZIGER der 5 Primär-CTAs ohne sichtbaren Tastatur-Fokusring (vom Review der vorigen Session aufgedeckt). Dieselben `focus-visible:ring`-Klassen wie die anderen 4 CTAs ergänzt (`focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background`) → Ring über die zentralen Tokens (`--ring`/`--background`), kein Hex, Dark-only, SSR-safe. WCAG 2.4.7 erfüllt; nur dieser eine Button angefasst. `npm run build` grün. **Browser-Check:** Tab auf den mobilen CTA → Ring sichtbar. |
| 09.06.2026 | **Session 16: CTA-Hover-Lift spürbarer (4px + Akzent-Lift-Schatten).** Den Hover-Lift der großen Content-CTAs (`main .cta-gradient` – Navbar weiter ohne Lift) deutlicher gemacht; Vorbild `hover:-translate-y-1` + `hover:shadow-md`, aber NUR Bewegung + Schatten übernommen, Farben über die Akzent-Tokens. Hover: `translateY(-4px)` (statt 2px) + zusätzlicher weicher Akzent-Schatten „oben drauf" als additiver box-shadow-Layer `var(--cta-lift-shadow)` (`0 12px 24px -8px color-mix(--accent 30%)` + Lightning-CSS-Token-Fallback; im Ruhezustand `0 0 #0000` → unsichtbar, daher Glow/Füllung/weiße Schrift UNVERÄNDERT). Klick (`:active`) drückt zurück auf die Grundlinie und der Lift-Schatten verschwindet. Glow per-State direkt deklariert (animiert verlässlich, kein Snap), Transition bleibt unlayered. reduced-motion: kein Lift (Transform + Lift-Schatten aus), Füllung statisch. Nur `src/index.css`, kein Button-Markup/keine Sektion; kein JS/magnetisches Folgen/Tilt, keine Navbar-Bewegung. **Adversariell reviewt** (7 Findings → 3 bestätigt, allesamt „verified correct"/keine Änderung nötig: Scope auf die 3 großen CTAs beschränkt + beide Navbar-CTAs ausgeschlossen; reduced-motion killt Lift + hält Füllung statisch; Glow/Füllung/weiße Schrift unverändert). `npm run build` grün; im `dist`-CSS verifiziert (Layer, `-4px`, `main`-Scope, color-mix `var(--accent)` + Fallback, kein graues/blaues Hex). **Offen/Follow-up:** Browser-Check (Lift spürbarer aber edel, Schatten violett nicht grau, Navbar ruhig, reduced-motion, keine Hydration-Warnung). |
| 09.06.2026 | **Session 15: Fließende CTA-Füllung + selektiver Lift.** Die 5 Primär-CTAs (`.cta-gradient`) bekommen eine langsam FLIESSENDE Gradient-Füllung (gefüllter Look bleibt, nur der Verlauf wandert) – Technik = der bewährte AuroraText-Weg: registrierte `@property --cta-pos` + `background-size:200%` + `background-position:var(--cta-pos)`, animiert via `@keyframes cta-flow` (9s alternate); NIE `background-position` direkt animiert (Clip-Drop-Bug; hier unkritisch da kein `background-clip:text`, aber bewusst derselbe Weg). Nur `--accent-gradient-strong` (dunkle Stops) → weiße Schrift über den GANZEN Fluss ≥4.5:1; Glow unverändert. **Selektiver Lift** nur auf den großen Content-CTAs (Hero, Kontakt-Submit, /möglichkeiten) via Selektor `main .cta-gradient` – Navbar-CTAs (in `<header>`) bleiben ruhig; Hover `translateY(-2px)` (touch-sicher `@media hover`), `:active` „drückt" auf die Grundlinie zurück (ungated). reduced-motion: Füllung statisch + kein Lift. Rein CSS-`@property`/`@keyframes` (kein JS) → SSR-sicher, Button-Markup unverändert. NUR `src/index.css` geändert (kein Button-Markup/keine Sektion angefasst); kein Wander-Rahmen, kein Puls-Glow. **Adversariell reviewt** (10 Findings → 4 bestätigt): 2 Kommentar-Härtungen angewandt (Hinweis, dass die `@property`-Indirektion hier bewusst/aus Konsistenz statt aus Zwang gewählt ist; Hinweis, dass die `.cta-gradient`-Transition unlayered bleiben muss, sonst „snappt" der Lift). **Bewusst NICHT gefixt** (Non-Goal Button-Markup): vorbestehende a11y-Lücke – der mobile Navbar-CTA hat keine `focus-visible:ring`-Klassen → als separater Follow-up notiert. `npm run build` grün; `@property`/`@keyframes`/Scope/Tokens im `dist`-CSS verifiziert, kein blaues Hex. **Offen/Follow-up:** Browser-Check (Fluss edel, weiße Schrift über den ganzen Verlauf lesbar, Lift nur große CTAs, Navbar ruhig, reduced-motion, keine Hydration-Warnung). |
| 09.06.2026 | **Session 14: Primär-CTA-Violett-Glow + Scroll-Fortschrittsbalken.** (1) **CTA-Glow** zentral in `.cta-gradient` (`src/index.css`) – dezenter, akzent-getönter Glow in DERSELBEN Sprache wie der `Card`-Hover-Glow (`color-mix(in oklab, var(--accent) x%, transparent)`): Ruhe ~18 %, Hover ~36 %, `:active` ~52 %; kein Puls/Dauer-Effekt, Hover via `@media (hover:hover)` touch-sicher. Greift automatisch für ALLE 5 Primär-CTAs (Navbar ×2, Hero, Kontakt-Submit, /möglichkeiten) – KEIN Button-Markup angefasst. `.cta-gradient` ist unlayered → die `box-shadow` bindet `var(--tw-ring-offset-shadow)`/`var(--tw-ring-shadow)` mit ein, damit der `focus-visible`-Ring (a11y) sichtbar bleibt (Glow + Ring komponieren). (2) **`ScrollProgress`** (`src/components/ScrollProgress.tsx`, in `Layout.tsx` gemountet): schmaler Balken `fixed top-0 h-[3px] z-[60]` über der Sticky-Navbar (`z-50`, `pointer-events-none`), Füllung `var(--accent-gradient)`, `scaleX` an framer `useScroll().scrollYProgress`. SSR-sicher (Scroll erst nach Mount, `scaleX` startet 0 → leerer Balken im prerenderten HTML, Server-Frame = erstes Client-Frame); reduced-motion ohne Spring direkt gebunden (bleibt sichtbar). Beide NUR Violett über Tokens, kein Hex hardcodiert. **Adversariell reviewt** (11 Findings → 4 bestätigt): **gefixt** – Spring-`skipInitialAnimation: true` (kein 0→Füllstand-Sweep bei Reload an gescrollter Position; Balken ist Info). **Bewusst belassen:** der von Lightning-CSS erzeugte opake `var(--accent)`-Fallback (für Engines ohne `color-mix`) – identisch zum bestehenden `Card`-Glow (konsistent) und ein expliziter alpha-Fallback würde Hex hardcodieren (verboten); das nun überschriebene `shadow-sm`/`transition-opacity` an den CTAs (harmlos, und Sektionen sollen unangetastet bleiben). `npm run build` grün (vor + nach Fix); Balken als `transform:scaleX(0)` vor dem `<header>` + token-basierter Glow im `dist`-CSS verifiziert. **Offen/Follow-up:** im Browser prüfen (Glow-Look + Fokusring-Sichtbarkeit, Balken-Lauf, reduced-motion, keine Hydration-Warnung). |
| 09.06.2026 | **Session 13: Live-Visual auf /möglichkeiten – Dot-Globe (WebGL) + Sparkles (Canvas).** Neuer Block auf der stillen Seite `/möglichkeiten` (zwischen Tilt und Marquee), reine Können-Demo „ich baue WebGL/Canvas-Visuals" – KEINE Startseiten-Änderung. Zwei neue UI-Komponenten: `Earth` (`src/components/ui/Earth.tsx`, interaktiver Dot-Globe via **`cobe`** v2, WebGL, Drag-to-rotate + sanfter Auto-Spin) und `Sparkles` (`src/components/ui/Sparkles.tsx`, aufsteigende 2D-Canvas-Funken, hand-gebaut ohne Paket). **NUR Violett:** Globus-Punkte/Glow + Funkenfarbe werden zur Laufzeit aus `--accent`/`--accent-solid` gelesen (für cobe per Hex→[r,g,b]-Parser); ALLE blauen cobe-Default-Hex + die englische Template-Copy raus. Block-Copy deutsch/„Du", kein „global"-/Reichweiten-Claim, keine erfundenen Zahlen (Label „Live im Browser gerechnet"). **SSR/Prerender-sicher:** im Render nur stabile `<canvas>`-Hüllen, alle window/WebGL/Canvas-Zugriffe im Effect; cobe dynamisch importiert (eigener Code-Split-Chunk ~13 KB, nie im Prerender). reduced-motion: Globus statisch (kein Spin), Sparkles ganz aus. rAF/Listener/Observer leak-frei beim Unmount abgebaut (wie CoolMode); cobe-`<style>` + WebGL-Context freigegeben. **Neue Dependency `cobe` (^2.0.1)** – erste Feature-Lib außerhalb des Kerns, in `package.json`/Lockfile committet (Vercel/`npm install` ziehen sie automatisch). **cobe v2 hat eine andere API als die Vorlage** (kein `onRender`, `update()` + eigene rAF-Schleife, Canvas-Re-Parenting) – aus der Quelle verifiziert und sauber adaptiert (eigenes Wrapper-`<div>` gegen React-`removeChild`-Fehler). **Adversariell reviewt** (Multi-Agent, 17 Findings → 8 bestätigt, alle 6 echten gefixt): leerer Globus unter reduced-motion (async Map-Textur → terminierte Redraws), `ResizeObserver` statt nur window-resize, `WEBGL_lose_context` beim Unmount, rAF-Pause off-screen via `IntersectionObserver`. `npm run build` grün (vor + nach den Fixes); `/möglichkeiten` prerendert mit Label + zwei dekorativen `<canvas>`-Hüllen verifiziert. **Offen/Follow-up:** im Browser prüfen (WebGL/Look, Perf, Drag, reduced-motion, keine Hydration-Warnung) – im grünen Build nicht sichtbar. |
| 09.06.2026 | **Session 12: Statement-Sektion (Scroll-Reveal) + wiederverwendbare `TextAnimation`.** Neue stille Sektion direkt vor „Kontakt" (`src/sections/Statement.tsx`, in `Home.tsx` zwischen `Prozess` und `Kontakt`): zwei nacheinander einblendende Zeilen „Nicht noch eine Seite von der Stange." / „Sondern deine." – „deine" im Akzent-Gradient über den zentralen Showcase-Helper `.accent-gradient-text` (statischer Clip, nichts hardcoden, NICHT vom AuroraText-`background-position`-Bug betroffen), Rest flach. Reveal-Mechanik in neuer wiederverwendbarer Komponente `src/components/TextAnimation.tsx` – Vorlagen-API nachgebaut (`text`/`as`/`letterAnime`/`lineAnime`/`direction`/`variants`/`classname`; blur + fade + y), an dieses Repo angepasst (kein Next/`'use client'`, `@/`-Alias, **kein** lowercase/capitalize-Transform – Text 1:1). **SSR-/hydration-sicher** nach den bekannten Regeln: `useReducedMotionSafe` (nicht framers `useReducedMotion`), Orchestrierung `useInView` + gesteuertes `animate` (nicht `whileInView`), Container `initial="hidden"` mit propagierten Kind-Varianten → beide Zeilen + `deine` stehen als echte Knoten im prerenderten `dist/index.html` (below-the-fold nur `opacity:0`, verifiziert), Server-Frame = erstes Client-Frame. Bewusst NICHT above-the-fold/Hero; andere Sektionen unangetastet, Dark-only/Responsive. **Adversariell reviewt** (Multi-Agent-Workflow, 11 Findings → 2 bestätigt, beide gefixt): (1) reduced-motion spielte trotz Kontrakt noch ein Fade/De-Blur/Slide ab, weil den reduced-Varianten `transition:{duration:0}` fehlte (jetzt ergänzt wie Prozess → sofort voll sichtbar, kein Blur/Stagger); (2) `lineAnime`-Prop war deklariert, aber ungenutzt (jetzt honoriert: Buchstaben › Zeilen › ein Block). `npm run build` grün (vor und nach den Fixes); prerenderter Inhalt + Gradient-Wort im `dist` verifiziert. **Offen/Follow-up:** im Browser prüfen (Reveal/reduced-motion/Hydration + „deine" während der Blur-Phase) – im grünen Build nicht sichtbar. |
