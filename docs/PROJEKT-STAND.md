...
- **Fließende CTA-Füllung + selektiver Lift (09.06.2026):** Die 5 Primär-CTAs (`.cta-gradient`) haben eine langsam FLIESSENDE Gradient-Füllung (gefüllter Look bleibt, nur der Verlauf wandert). **Technik = der bewährte AuroraText-Weg:** registrierte `@property --cta-pos` (`<percentage>`) + `background-size:200% 100%` + `background-position:var(--cta-pos) 50%`, animiert via `@keyframes cta-flow` (9s ease-in-out alternate) – es wird NUR die `@property` animiert, NIE `background-position` direkt (genau der Chromium-Clip-Drop-Bug; hier unkritischer, da der Button KEIN `background-clip:text` hat, aber derselbe sichere Weg). Füllung bewusst NUR `--accent-gradient-strong` (dunkle Stops) → weiße Schrift bleibt über den GANZEN Fluss ≥4.5:1; die hellen Stops NICHT einziehen. Glow (Ruhe/Hover/`:active`) unverändert. **Selektiver Lift:** nur die großen Content-CTAs (Hero, Kontakt-Submit, /möglichkeiten) heben bei Hover leicht an (`transform: translateY(-2px)`) und „drücken" sich beim Klick zurück auf die Grundlinie (`:active → translateY(0)`); die Navbar-CTAs bleiben ruhig. Scope ohne Button-Markup-Änderung über den Selektor **`main .cta-gradient`** (die großen CTAs liegen in `<main>`, die Navbar-CTAs in `<header>`). Hover touch-sicher (`@media (hover:hover)`), `:active` ungated (Tap); `transform` läuft über die `.cta-gradient`-Transition weich. **reduced-motion:** Füllung statisch (`animation:none`, `background-position:0% 50%`) UND kein Lift (`transform:none`). Rein CSS-`@keyframes`/`@property` (kein JS) → SSR-sicher, Button-Markup im prerenderten HTML unverändert. KEIN Wander-Rahmen (`HoverBorderGradient` bewusst nicht gebaut – wir bleiben gefüllt), kein Puls-Glow. Nur in `src/index.css`.
